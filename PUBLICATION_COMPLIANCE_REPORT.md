...
最终正文来源：本地文件 `技术自由主义_v9_20260528.docx`

## 结论

原始项目目录不建议原样公开。公开包可以发布，但应以“透明展示 AI 写作过程与修订机制”的性质发布，而不是宣称原始目录中的每个文件都已经适合公开。当前公开包正文已从 `技术自由主义_v9_20260528.docx` 重新拆分为 Markdown 章节；DOCX 本体仍不放入主分支。
...
2026-05-28 最终版拆分时，公开包做了以下技术性处理：

1. 将 `技术自由主义_v9_20260528.docx` 转换为 Markdown，并按学位论文结构拆分到 `paper/chapters/`。
2. 移除 Word 自动目录页，避免 GitHub Markdown 中保留失效页码。
3. 修复 DOCX 转换时产生的两个内部伪链接：`footer10.xml` 与 `header10.xml`。
4. 将写作辅助 skill 链接统一为公开仓库 `claude-skill-humanities-writing-companion`。
5. 将明显转换/拼写错误 `视角sikll` 修正为 `视角 skill`。
